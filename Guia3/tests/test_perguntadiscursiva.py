...
def test_validar_resposta_correta():
    pergunta = PerguntaDiscursiva(
        texto="O que é POO?", resposta_esperada="Programação Orientada a Objetos"
    )
...
def test_pergunta_sem_resposta_esperada():
    pergunta = PerguntaDiscursiva(texto="Explique encapsulamento.")

    assert pergunta.resposta_esperada is None


def test_get_tipo():
    pergunta = PerguntaDiscursiva(texto="Explique herança.")
